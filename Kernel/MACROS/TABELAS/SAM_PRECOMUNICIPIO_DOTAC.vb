'HASH: C85F5E3F7545372E365C9D23FDC5E5C3
'Macro: SAM_PRECOMUNICIPIO_DOTAC
'#Uses "*ProcuraEvento"
'#Uses "*bsShowMessage"
...
' Paulo Melo - SMS 118697 - 01/10/2009 - Fim
	...
	If SQL.FieldByName("NIVEL1").AsInteger = 6 Then
		Nivel = 1
	ElseIf SQL.FieldByName("NIVEL2").AsInteger = 6 Then
		Nivel = 2
	ElseIf SQL.FieldByName("NIVEL3").AsInteger = 6 Then
		Nivel = 3
	ElseIf SQL.FieldByName("NIVEL4").AsInteger = 6 Then
		Nivel = 4
	ElseIf SQL.FieldByName("NIVEL5").AsInteger = 6 Then
		Nivel = 5
	ElseIf SQL.FieldByName("NIVEL6").AsInteger = 6 Then
		Nivel = 6
	ElseIf SQL.FieldByName("NIVEL7").AsInteger = 6 Then
		Nivel = 7
	ElseIf SQL.FieldByName("NIVEL8").AsInteger = 6 Then
		Nivel = 8
	End If
	...
	If Nivel <> -1 Then
		...
		Else
			...
			ValorEvento = Interface.ValorEvento(CurrentSystem, vDataBaseChecagemVigencia, 99, -1, -1, -1, -1, -1, -1, CurrentQuery.FieldByName("MUNICIPIO").Value, -1, CurrentQuery.FieldByName("EVENTO").Value, -1, Nivel, CurrentQuery.FieldByName("CONVENIO").AsInteger, "", CurrentQuery.FieldByName("CBOS").AsString)

			result = "Valor do evento nesta vigência: R$ " + Format(ValorEvento, "#,##0.0000")+" ("+Format(ValorEvento,"#,##0.00")+")"
		End If
	Else
		result = "Na configuração de busca de preço, não foi definido um nível para o Muncipio!"
	End If
...
Public Sub BOTAOREPLICARPORCBOS_OnClick()
  ...
  SessionVar("TabelaDeDotacao") = "SAM_PRECOMUNICIPIO_DOTAC"
...
Public Sub TABLE_BeforePost(CanContinue As Boolean)
	...
	Linha = Interface.Vigencia(CurrentSystem, "SAM_PRECOMUNICIPIO_DOTAC", "DATAINICIAL", "DATAFINAL", CurrentQuery.FieldByName("DATAINICIAL").AsDateTime, CurrentQuery.FieldByName("DATAFINAL").AsDateTime, "MUNICIPIO", Condicao)
