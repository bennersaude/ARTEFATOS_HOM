'HASH: 6E3EE6E82309360384548A1F6056EDAE
...
Public Sub TABLE_AfterPost()
	...
	If pegarTipoAcomodacao="E" Then
		resultado = dll.gerarProrrogacaoAfterPost(CurrentSystem, CurrentQuery.FieldByName("acomodacaoEvento").AsInteger, CurrentQuery.FieldByName("CODIGOTABELA").AsInteger,msg)
	Else
		resultado = dll.gerarProrrogacaoAfterPost(CurrentSystem, CurrentQuery.FieldByName("acomodacaoGrau").AsInteger, CurrentQuery.FieldByName("CODIGOTABELA").AsInteger, msg)
	End If
    ...
    Else
      If WebMode Then
		...
		spBSAUT_GERARPROTOCOLOTRANSACAO.ParamByName("P_NUMEROPROTOCOLOTRANSACAO").AsInteger = 0 ' PARA SEMPRE GERAR UM NOVO PROTOCOLO DE TRNSAÇÃO
		...
		SessionVar("HANDLEPROTOCOLOTRANSACAO") = spBSAUT_GERARPROTOCOLOTRANSACAO.ParamByName("P_NUMEROPROTOCOLOTRANSACAO").AsString
...
Public Sub TABLE_BeforeInsert(CanContinue As Boolean)
	...
	If WebMode Then
		...
		resultado = dll.GerarProrrogacaoNewRecord(CurrentSystem, acomodacoes, msg)
		Set dll=Nothing
...
Public Sub TABLE_BeforePost(CanContinue As Boolean)
    ...
    If pegarTipoAcomodacao="E" And CurrentQuery.FieldByName("ACOMODACAOEVENTO").IsNull   Then
		bsShowMessage("Obrigatório o preenchimento do campo de acomodação por evento!", "E")
		CanContinue = False
		Exit Sub
	ElseIf pegarTipoAcomodacao="G" And CurrentQuery.FieldByName("ACOMODACAOGRAU").IsNull Then
		bsShowMessage("Obrigatório o preenchimento do campo de acomodação por grau!", "E")
		CanContinue = False
		Exit Sub
	...
	sql.Clear
	sql.Add("UPDATE SAM_AUTORIZ SET PRORROGACAOLIBERADA = :L, PRORROGACAOSOLICITADA= :S WHERE HANDLE=:H")
	sql.ParamByName("H").AsInteger = CLng(SessionVar("HANDLEAUTORIZACAO"))
...
Public Sub comum
	Dim sql As BPesquisa
	Set sql = NewQuery
	sql.Add("SELECT PRORROGACAOMAXTGE, PRORROGACAOSOLICITADA, PRORROGACAOLIBERADA FROM SAM_AUTORIZ WHERE HANDLE=:H")
	sql.ParamByName("H").AsInteger=CLng(SessionVar("HANDLEAUTORIZACAO"))
	sql.Active=True
	CurrentQuery.FieldByName("qtdtge").AsInteger = sql.FieldByName("prorrogacaomaxtge").AsInteger
	CurrentQuery.FieldByName("QTDLIBERADA").AsInteger = sql.FieldByName("PRORROGACAOLIBERADA").AsInteger
	CurrentQuery.FieldByName("QTDSOLICITADA").AsInteger = sql.FieldByName("PRORROGACAOSOLICITADA").AsInteger
	Set sql=Nothing
End Sub
