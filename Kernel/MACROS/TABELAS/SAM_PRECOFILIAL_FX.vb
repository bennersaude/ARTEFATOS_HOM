'HASH: 563DC8576BA5C154EE5853B0A333DB3B
'Macro: SAM_PRECOFILIAL_FX
'#Uses "*bsShowMessage"
'#Uses "*ProcuraEvento"
...
Public Sub EVENTOINICIAL_OnPopup(ShowPopup As Boolean)
	...
	ElseIf WebMode Then
		vsCriterio = " MASCARATGE = @CAMPO(MASCARATGE)"
	End If
...
Public Sub TABLE_BeforePost(CanContinue As Boolean)
	...
	If CanContinue = True Then
		...
		Condicao = " FILIAL = " + CurrentQuery.FieldByName("FILIAL").AsString
		...
		Linha = Interface.EventoFx(CurrentSystem, "SAM_PRECOFILIAL_FX", CurrentQuery.FieldByName("DATAINICIAL").AsDateTime, CurrentQuery.FieldByName("DATAFINAL").AsDateTime, EstruturaI, EstruturaF, Condicao)
...
Public Sub BOTAOVALORES_OnClick()
	...
	If Nivel <> -1 Then
		If Not CurrentQuery.FieldByName("TABELAPRECO").IsNull Then
			...
			Interface.ValoresFxEventos(CurrentSystem, 15, CurrentQuery.FieldByName("FILIAL").Value, -1, CurrentQuery.FieldByName("ESTRUTURAINICIAL").Value, CurrentQuery.FieldByName("ESTRUTURAFINAL").Value, -1, CurrentQuery.FieldByName("CONVENIO").AsInteger, CurrentQuery.FieldByName("HANDLE").AsInteger, CurrentQuery.FieldByName("MASCARATGE").AsInteger)
		Else
			bsShowMessage("Para visualizar os preços dos eventos desta configuração de faixa, a tabela genérica deve ser informada ! ", "I")
		End If
	Else
		bsShowMessage("Na configuração de busca do preço, não foi definido um nível para o Filial !", "I")
	End If
End Sub
