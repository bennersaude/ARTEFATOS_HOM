'HASH: 867D49BDC50F88F55F5A0D6F5EA4EC5A
'Macro: SAM_PRECOGERALREGIME_DOTAC
...
' Paulo Melo - SMS 118697 - 01/10/2009 - Fim
	...
	Else
		...
		ValorEvento = Interface.ValorEvento(CurrentSystem, vDataBaseChecagemVigencia, 10, -1, -1, -1, -1, -1, -1, -1, -1, CurrentQuery.FieldByName("EVENTO").Value, CurrentQuery.FieldByName("REGIMEATENDIMENTO").Value, 9, CurrentQuery.FieldByName("CONVENIO").AsInteger, "", CurrentQuery.FieldByName("CBOS").AsString)

		If VisibleMode Then
			LABELPRECO.Text = "Valor do Evento: R$ " + Format(ValorEvento, "#,##0.0000")+" ("+Format(ValorEvento,"#,##0.00")+")"
		Else
			bsShowMessage("Valor do Evento: R$ " + Format(ValorEvento, "#,##0.0000")+" ("+Format(ValorEvento,"#,##0.00")+")", "I")
		End If
	End If
End Sub
...
Public Sub BOTAOREPLICARPORCBOS_OnClick()
  ...
  SessionVar("TabelaDeDotacao") = "SAM_PRECOGERALREGIME_DOTAC"
...
Public Sub TABLE_BeforePost(CanContinue As Boolean)
	...
	If CurrentQuery.FieldByName("CBOS").IsNull Then
		Condicao = Condicao + " AND CBOS IS NULL"
	Else
		Condicao = Condicao + " AND CBOS = " + CurrentQuery.FieldByName("CBOS").AsString
	End If

	Linha = Interface.Vigencia(CurrentSystem, "SAM_PRECOGERALREGIME_DOTAC", "DATAINICIAL", "DATAFINAL", CurrentQuery.FieldByName("DATAINICIAL").AsDateTime, CurrentQuery.FieldByName("DATAFINAL").AsDateTime, "REGIMEATENDIMENTO", Condicao)
