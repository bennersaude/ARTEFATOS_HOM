'HASH: 9028268935D78ADD9CA880ECF6E31D68
'#Uses "*bsShowMessage"
Option Explicit
...
Public Sub EVENTO_OnPopup(ShowPopup As Boolean)
  If VisibleMode Then
    ...
    vCriterio = "INCLUINOTIPOANEXO = 'O' AND ULTIMONIVEL = 'S' AND INATIVO <> 'S' AND EXISTS(SELECT 1 FROM SAM_TGE_TABELATISS T WHERE T.EVENTO = SAM_TGE.HANDLE AND T.TABELATISS = " + CurrentQuery.FieldByName("CODIGOTABELA").AsString + ")"
...
Public Sub TABLE_AfterScroll()
  ...
  vSQL.Add("FROM SAM_AUTORIZ_ANEXOOPME ")
  vSQL.Add("WHERE HANDLE = :ANEXOOPME ")
  vSQL.ParamByName("ANEXOOPME").Value = CurrentQuery.FieldByName("ANEXOOPME").AsInteger

  vSQL.Active = True

  CODIGOTABELA.WebLocalWhere = "A.VERSAOTISS = "+ CStr(PegarHandleVersaoTISS(vSQL.FieldByName("AUTORIZACAO").AsInteger))
  EVENTO.WebLocalWhere = "A.INCLUINOTIPOANEXO = 'O' AND A.ULTIMONIVEL = 'S' AND A.INATIVO <> 'S' AND EXISTS(SELECT 1 FROM SAM_TGE_TABELATISS T WHERE T.EVENTO = A.HANDLE AND T.TABELATISS = @CAMPO(CODIGOTABELA))"
