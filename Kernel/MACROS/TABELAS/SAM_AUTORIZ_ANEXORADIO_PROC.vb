'HASH: 8C4F309FBF4F11FA7F62C5825F06F77A
'#Uses "*bsShowMessage"
Option Explicit
...
Public Sub EVENTO_OnPopup(ShowPopup As Boolean)
  If VisibleMode Then
    ...
    vCriterio = "INCLUINOTIPOANEXO = 'R' AND ULTIMONIVEL = 'S' AND INATIVO <> 'S' AND EXISTS(SELECT 1 FROM SAM_TGE_TABELATISS T WHERE T.EVENTO = SAM_TGE.HANDLE AND T.TABELATISS = " + CurrentQuery.FieldByName("CODIGOTABELA").AsString + ")"
...
Public Sub TABLE_AfterScroll()
  ...
  vSQL.Add("FROM SAM_AUTORIZ_ANEXORADIO ")
  vSQL.Add("WHERE HANDLE = :ANEXORADIOTERAPIA ")
  vSQL.ParamByName("ANEXORADIOTERAPIA").Value = CurrentQuery.FieldByName("ANEXORADIOTERAPIA").AsInteger

  vSQL.Active = True

  CODIGOTABELA.WebLocalWhere = "A.VERSAOTISS = "+ CStr(PegarHandleVersaoTISS(vSQL.FieldByName("AUTORIZACAO").AsInteger))
  EVENTO.WebLocalWhere = "A.INCLUINOTIPOANEXO = 'R' AND A.ULTIMONIVEL = 'S' AND A.INATIVO <> 'S' AND EXISTS(SELECT 1 FROM SAM_TGE_TABELATISS T WHERE T.EVENTO = A.HANDLE AND T.TABELATISS = @CAMPO(CODIGOTABELA))"
