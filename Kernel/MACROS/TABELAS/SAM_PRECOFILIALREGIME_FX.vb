'HASH: B5EF6EA2BD7D7C2A9DB844310918365E
'Macro: SAM_PRECOFILIALREGIME_FX
'#Uses "*ProcuraEvento"
'#Uses "*bsShowMessage"
...
Public Sub TABLE_BeforePost(CanContinue As Boolean)
	...
	Linha = Interface.EventoFx(CurrentSystem, "SAM_PRECOFILIALREGIME_FX", CurrentQuery.FieldByName("DATAINICIAL").AsDateTime, CurrentQuery.FieldByName("DATAFINAL").AsDateTime, EstruturaI, EstruturaF, Condicao)
...
Public Sub BOTAOVALORES_OnClick()
	...
	If Nivel <> -1 Then
		If Not CurrentQuery.FieldByName("TABELAPRECO").IsNull Then
			...
			Interface.ValoresFxEventos(CurrentSystem, 15, CurrentQuery.FieldByName("FILIAL").Value, -1, CurrentQuery.FieldByName("ESTRUTURAINICIAL").Value, CurrentQuery.FieldByName("ESTRUTURAFINAL").Value, CurrentQuery.FieldByName("REGIMEATENDIMENTO").Value, CurrentQuery.FieldByName("CONVENIO").AsInteger, CurrentQuery.FieldByName("HANDLE").AsInteger, CurrentQuery.FieldByName("MASCARATGE").AsInteger)
		Else
			bsShowMessage("Para visualizar os preços dos eventos desta configuração de faixa, a tabela genérica deve ser informada !", "I")
		End If
	Else
		bsShowMessage("Na configuração de busca do preço, não foi definido um nível para a Filial !", "I")
	End If
End Sub
